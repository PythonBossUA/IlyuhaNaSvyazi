...
GET_LAST_ENCRYPTED_MESSAGES_LIMIT = 15
...
async def get_last_encrypted_messages(database, client_id, key, last_sent_at=None, last_message_id=None):
    stmt = (
        ...
        .limit(GET_LAST_ENCRYPTED_MESSAGES_LIMIT + 1)
    )
    if last_sent_at and last_message_id:
        stmt = stmt.where(or_(
    ...
    has_more = len(rows) > GET_LAST_ENCRYPTED_MESSAGES_LIMIT
    view_rows = rows[:GET_LAST_ENCRYPTED_MESSAGES_LIMIT] if has_more else rows
    ...
    if has_more:
        oldest = rows[GET_LAST_ENCRYPTED_MESSAGES_LIMIT - 1]
...
async def websocket_endpoint(websocket: WebSocket, database: DATABASE, client_id: str):
    ...
    try:
        ...
        while not authenticated:
            ...
            if auth.get("type") == "authorization":
        ...
        if ws_user.require_password_change:
            ...
            while not pw_changed:
                ...
                if cd.get("type") == "password_change":
